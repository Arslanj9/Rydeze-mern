...
const Commuters = () => {
  ...
  useEffect(() => {
    const fetchCommuterData = async () => {
      try {
        setLoading(true);
        const publishResponse = await axios.get(`${apiUrl}/api/publish/getCommuter`);
        const publishData = publishResponse.data;

        const temp = publishData.map((publish) => console.log(`PublishID is: ${JSON.stringify(publish)}`))
        ...
        const commuterDetailsPromises = publishData.map(async (commuter) => {
          try {
            ...
            const userResponse = await axios.post(`${apiUrl}/api/users/getUserData`, {
  ...
  return (
    <div className="p-3 sm:p-6 max-w-lg mx-auto">
      ...
      <h1 className='text-2xl text-center mb-5 p-3 rounded-lg bg-[#292d49]'>Commuters</h1>

      {selectedCommuter ? (
        <CommuterDetails commuter={selectedCommuter} onClose={handleCloseDetails} />
      ) : commuterData.length > 0 ? (
        commuterData.map((commuter) => (
          <div className='hover:cursor-pointer flex flex-row' onClick={() => handleCardClick(commuter)} key={commuter._id}>
            <CommuterCard
              name={commuter.name}
              ...
              departureTime={commuter.departureTime}
              departureDate={commuter.departureDate}
              numberOfRequiredSeats={commuter.numberOfRequiredSeats}
            />
          </div>
        ))
      ) : (
        <div className='border rounded-xl border-white border-opacity-55'>
          <h1 className='text-center m-10'>There are no commuters at the moment. Please check again later.</h1>
        </div>
      )}
    </div>
  );
