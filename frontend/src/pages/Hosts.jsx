...
const Hosts = () => {
  ...
  useEffect(() => {
    const fetchHostsData = async () => {
      try {
        ...
        const publishResponse = await axios.get(`${apiUrl}/api/publish/getHost`);
        ...
        const hostDetailsPromises = publishData.map(async (publish) => {
          ...
          const vehicleResponse = await axios.post(`${apiUrl}/api/users/getVehicle`, {
  ...
  return (
    <div className="p-3 sm:p-6 max-w-lg mx-auto">
      ...
      <h1 className='text-2xl text-center mb-5 p-3 rounded-lg bg-[#292d49]'>Rides</h1>
      ...
      {selectedHost ? (
        ...
        <div className='border rounded-xl border-white border-opacity-55'>
          <h1 className='text-center m-10'>There are no ride at the moment. Please check again later.</h1>
        </div>
      )}
